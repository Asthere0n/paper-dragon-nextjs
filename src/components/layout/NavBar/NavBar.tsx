...
export default function NavBar({ selector }: { selector: (view: string) => void }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 w-full px-2 pb-[calc(env(safe-area-inset-bottom)+0.65rem)] pt-2 sm:sticky sm:px-0 sm:pb-4">
        <ul className={`${style.nav} relative mx-auto flex w-fit max-w-full items-center justify-center gap-3 rounded-[1.55rem] border-[3px] border-black bg-purple-800 px-2 py-2 text-black shadow-[4px_4px_0_#111] sm:gap-7 sm:rounded-2xl sm:border-4 sm:px-4 sm:py-3 sm:shadow-[7px_7px_0_#111]`}>
