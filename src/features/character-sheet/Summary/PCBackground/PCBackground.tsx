...
export default function PCBackground({ customStyles }: Props) {
  ...
  return (
    <div className={`${styles.vignette} row-start-1 row-end-2 flex flex-col items-center text-center`} style={customStyles}>
      ...
      <ul className="flex flex-wrap justify-center gap-2 px-3 pb-4 sm:gap-3 sm:px-4">{background.abilityScores.map((ability: AbilityName)=>(
        <li className={`${abilityPillColors[ability]} ${abilityPillClassName}`} key={ability}>{abilityLabels[ability]}</li>
      ))}</ul>
    </div>
  )
}
